...
export default function ReadItem({ config, selectedItem }) {
  ...
  return (
    <>
      <PageHeader
        ...
        tags={[
          ...
          currentErp.paymentStatus && (
            <span key="paymentStatus">{currentErp.paymentStatus && translate(currentErp.paymentStatus)}</span>
          ),
        ]}
        extra={[
          <Button
          ...
          <Button
            ...
            icon={<MailOutlined />}
            disabled={!currentErp.client?.email}
          >
            {translate('Gửi qua Email')}
          </Button>,
          <Button
            key={`${uniqueId()}`}
            onClick={() => {
      ...
      >
        <Row>
          <Statistic title={translate('Trạng thái')} value={currentErp.status} />
          <Statistic
      ...
      <Divider dashed />
      <Descriptions title={`Khách hàng: ${currentErp.client.name}`}>
        <Descriptions.Item label={translate('Địa chỉ')}>{client.address}</Descriptions.Item>
        <Descriptions.Item label={translate('Email')}>{client.email}</Descriptions.Item>
        <Descriptions.Item label={translate('Điện thoại')}>{client.phone}</Descriptions.Item>
      </Descriptions>
      <Divider />
      <Row gutter={[12, 0]}>
        <Col className="gutter-row" span={11}>
          <p><strong>{translate('Sản phẩm')}</strong></p>
        </Col>
        <Col className="gutter-row" span={4}>
          <p style={{ textAlign: 'right' }}><strong>{translate('Giá')}</strong></p>
        </Col>
        <Col className="gutter-row" span={4}>
          <p style={{ textAlign: 'right' }}><strong>{translate('Số lượng')}</strong></p>
        </Col>
        <Col className="gutter-row" span={5}>
          <p style={{ textAlign: 'right' }}><strong>{translate('Tổng cộng')}</strong></p>
        </Col>
        <Divider />
      </Row>
      {itemslist.map((item) => (
